...
class GolfBallFollower:
    ...
    def stop_ball_follower(self):
        mqtt_service.send_cmd("ti/rc", f"{0} {0} {time()}")

        
    def setup(self, camera_service):
        self.logger = Logger('golf_ball_follower')
        self.set_ball_follower(0)
        self.camera_service = camera_service
        
        self.camera_service = camera_service

        self.logger.info('Golf ball follower started')
    ...
    def calculate_turn_speed(self):
        ...
        print(f'Calculating turn speed x:{ball_x}; z_mm:{ball_z_mm}')
        if not ball_detected:
            print('Not detecting the ball')
        ...
        self.z_history.append(ball_z_mm)
        ...
        # Calculate turn speed normally if the ball is detected but not too close
    ...
    def set_ball_follower(self, velocity):
        if self.velocity == 0 and velocity != 0:
            self.reset()
            self.camera_service.set_golf_ball_detection(True)
